...
for (let i = 0; i < pages.length; i++)
  pages[i].outputPath =
    pages[i].outputPath === undefined ? pages[i].source : pages[i].outputPath;

const config = {
  entry: Object.fromEntries(
    pages.map((page) => [page.keyName, `${paths.pages}${page.source}/index.ts`])
  ),
    ...
    new FileManagerPlugin({
      ...
      events: {
        ...
        onEnd: {
          copy: [
            {
              source: `${paths.src}/assets/to-root`,
